...
Repository branch protection must require the `Quality (ubuntu-latest)`, `Quality (windows-latest)`, `MSRV (Rust 1.85.0)`, `Dependency policy and audit`, `Secret scan`, and `SBOM` checks before merging into a protected release branch.
