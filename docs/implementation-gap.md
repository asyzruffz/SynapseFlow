...
| Toolchain contract | Workspace package metadata declares Rust `1.90`, while the platform ADR and CI MSRV job require Rust `1.87`. | Restore one authoritative MSRV across Cargo metadata, the pinned toolchain, documentation, and CI, or explicitly redesign the platform contract through an ADR. |
