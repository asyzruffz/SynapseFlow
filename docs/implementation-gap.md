...
- The CLI composes the Crux kernel with the application-owned generation
  ...
  CPU-only llama.cpp adapter for the verified-local profile, and local Loom
  workers with bounded loopback transport for the schema-v2 profile. The
  orchestrator selects a runtime only from a verified manifest.
- Loom implements the Llama `layer_range_v1` shard-execution adapter with
  ...
  cancellation, deadline checks, embedded-tokenizer encoding/decoding, and
  terminal session release. The application-owned sharded runtime plans a
  verified schema-v2 route, drives canonical loopback frames through the Loom
  stages, records checkpoints, samples validated logits, and releases runtime
  state. The loopback harness exercises a contiguous baseline, two ranges, a
  checkpoint, recovery, and prompt-to-token generation.
...
| Provisioned runtime validation | The checked-in sharded composition test covers generated fixtures. There is no signed external schema-v2 fixture workflow or Linux validation profile for either supported runtime profile. | Add provisioned schema-v2 fixture validation through the production sharded composition root and validate the supported runtime profiles on every Tier-1 platform before making cross-platform release claims. |
