...
The current checked-out TinyLlama GGUF has a known artifact hash and upstream provenance, but it is not a source-controlled test asset and has no SynapseFlow manifest or trusted publisher signature. Milestone 2 needs a precise external fixture and a manifest profile before acquisition and backend implementation begin.
...
- Keep the fixture-signing key dedicated to non-production integration testing. Production trust stores exclude it. The canonical signed manifest and immutable expected token vector are generated through the documented Step 3/Step 5 procedure, not guessed or checked into the default test path.
