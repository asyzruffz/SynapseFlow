# Loopback-sharding Windows validation profile

> **Source of truth.** This profile defines Windows loopback-sharding
> validation. Change it only through an explicit validation redesign; the
> runtime must conform to its stated compatibility and recovery requirements.

This profile defines the provisioned TinyLlama GGUF validation on Windows. It
contains no prompt text, token values, activation values, weights, cache paths,
or credentials.
...
| Supplied manifest provenance | Signed verified-local-inference schema-v1 fixture reference `registry://fixtures/synapseflow-verified-local-tinyllama-q5km-v1@sha256:8c9c17c57eed25a84f908c6a72a24d90d37546713d4480aba2e3dadb5d0e29e5` |
| Loom declaration | In-memory schema-v2, two-range `[0, 11)` → `[11, 22)` declaration derived only for this validation. It is not a signed loopback-sharding manifest. |
...
The artifact hash and size match the supplied immutable fixture metadata.
...
exactly; the replica completes with exactly one retry and one fallback.

## Scope boundary

This profile is Windows-specific and must not be used to claim cross-platform
performance. It lacks a signed schema-v2 Loom fixture manifest; an immutable
signed schema-v2 declaration is required before making a production performance
claim. Compression comparison requires a separately versioned compression
capability.
