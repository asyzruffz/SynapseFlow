...
- [x] (2026-08-23: [`GenerationService`](application/src/lib.rs), legacy crate cleanup) Put the generation use case in the application layer and make the CLI/node call it through ports. Remove the legacy Candle/safetensors path so it cannot be used as supported behavior.
